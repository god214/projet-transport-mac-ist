...
export default function AdminPage() {
  ...
  const handlePrintPDF = () => {
    // On cache temporairement les éléments non voulus dans le PDF
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200 no-print">
          <h1 className="text-3xl font-bold text-center mb-3 text-gray-800">Espace Administrateur</h1>
          <p className="text-center text-gray-600 mb-8">Président de la Mutuelle Action Collective</p>
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-lg px-5 py-4 mb-6 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
          />
    ...
    <div className="min-h-screen bg-gray-50 p-6 print-area">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4 no-print">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Administration Projet Transport</h1>
            <p className="text-gray-600 mt-2">
              Total inscrits : <span className="font-semibold text-blue-600">{totalCount}</span>
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow border border-gray-200 p-6 mb-8 no-print">
          <div className="flex flex-col sm:flex-row gap-4">
            <input
              ...
              className="flex-1 border border-gray-300 rounded-lg px-5 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            <button
        ...
        {/* Résultats */}
        {loading ? (
          <div className="text-center py-20 text-gray-500 text-lg">Chargement...</div>
        ) : etudiants.length === 0 ? (
          <div className="bg-white rounded-xl shadow border border-gray-200 p-12 text-center text-gray-600">
            <p className="text-xl">Aucun inscrit trouvé</p>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-800 text-white">
                  <tr>
                    <th className="px-6 py-5 text-left text-sm font-semibold uppercase tracking-wider">#</th>
                    <th className="px-6 py-5 text-left text-sm font-semibold uppercase tracking-wider">Nom</th>
                    <th className="px-6 py-5 text-left text-sm font-semibold uppercase tracking-wider">Prénom</th>
                    <th className="px-6 py-5 text-left text-sm font-semibold uppercase tracking-wider">Téléphone</th>
                    <th className="px-6 py-5 text-left text-sm font-semibold uppercase tracking-wider">Quartier</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {etudiants.map((etudiant, index) => (
                    <tr key={index} className="hover:bg-blue-50 transition-colors">
                      <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-500">{index + 1}</td>
                      <td className="px-6 py-5 whitespace-nowrap font-medium text-gray-900">{etudiant.nom}</td>
                      <td className="px-6 py-5 whitespace-nowrap text-gray-900">{etudiant.prenom}</td>
                      <td className="px-6 py-5 whitespace-nowrap text-gray-600">
                        {etudiant.numero_telephone || '—'}
                      </td>
                      <td className="px-6 py-5 whitespace-nowrap text-gray-600">{etudiant.quartier}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="text-center text-gray-500 text-sm mt-12 no-print">
          Institut Supérieur de Technologie & Mutuelle Action Collective – Projet Transport – {new Date().getFullYear()}
        </div>
      </div>
    </div>
  );
